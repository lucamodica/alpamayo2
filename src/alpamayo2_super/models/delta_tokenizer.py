...
class DeltaTrajectoryTokenizer:
    ...
    def __init__(
        ...
        pad_origin_at_beginning: bool = True,
    ):
        """Initialize the tokenizer.

        Args:
            ego_xyz_min: Minimum XYZ coordinates for the ego vehicle.
            ego_xyz_max: Maximum XYZ coordinates for the ego vehicle.
            ego_yaw_min: Minimum yaw angle for the ego vehicle.
            ego_yaw_max: Maximum yaw angle for the ego vehicle.
            num_bins: Number of bins for the XYZ coordinates.
            predict_yaw: Whether to predict the yaw angle.
            load_weights: Whether to load the weights.
            pad_origin_at_beginning: True encodes a future trajectory anchored at a
                prepended ego origin, so T waypoints produce T deltas. False encodes
                history ending at the ego origin, so T waypoints produce T-1 deltas.
        """
        ...
        self.pad_origin_at_beginning = pad_origin_at_beginning
    ...
    def encode(
        ...
        if self.pad_origin_at_beginning:
        ...
        # Calculate delta yaw with the same anchoring convention as XYZ.
        if self.pad_origin_at_beginning:
    ...
    def decode(
        ...
        if self.pad_origin_at_beginning:
            ...
            fut_xyz = -torch.flip(torch.cumsum(torch.flip(xyz, dims=[1]), dim=1), dims=[1])
            fut_xyz = torch.cat([fut_xyz, torch.zeros_like(fut_xyz[:, :1])], dim=1)
        if not self._predict_yaw:
        ...
        if self.pad_origin_at_beginning:
            ...
            yaw = -torch.flip(torch.cumsum(torch.flip(yaw, dims=[1]), dim=1), dims=[1])
            yaw = torch.cat([yaw, torch.zeros_like(yaw[:, :1])], dim=1)
